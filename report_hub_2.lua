...
SelectedPlayerText.Size = UDim2.new(0.65, -15, 1, -10)
...
-- Dropdown UI and logic removed. Player selection is now a simple TextBox with autofill logic.

-- Autofill Logic
local function findBestMatch(partialName)
    local lowerPartial = string.lower(partialName)
    local bestMatch = nil
    local players = Players:GetPlayers()

    if lowerPartial == "" or lowerPartial == "select or enter name" then
        return nil
    end

    -- 1. Exact match (case-insensitive)
    for _, player in ipairs(players) do
        if string.lower(player.Name) == lowerPartial then
            return player.Name -- Found exact match, return immediately
        end
    end
    ...
    -- 3. Contains match (case-insensitive) - fallback if no starts-with
    if not bestMatch then
        for _, player in ipairs(players) do
            -- Use plain string.find for case-insensitive contains match
...
SelectedPlayerText.FocusLost:Connect(function(enterPressed)
    local currentText = SelectedPlayerText.Text
    local matchedName = findBestMatch(currentText)

    if matchedName then
        SelectedPlayerText.Text = matchedName
    elseif currentText ~= "Select or enter name" and currentText ~= "" then
        print("No player found matching: " .. currentText)
    end
end)
